...
Eigen::Matrix3d strainVoigt2Mat(const Vector6d& strain_vec)
{
    ...
    strain_mat <<       strain_vec[0], 0.5*strain_vec[3], 0.5*strain_vec[4],
                        0.5*strain_vec[3], strain_vec[1], 0.5*strain_vec[5],
                        0.5*strain_vec[4], 0.5*strain_vec[5], strain_vec[2];
...
Vector6d strainMat2Voigt(const Eigen::Matrix3d& strain_mat)
{
    ...
    strain_vec[3] = 2*strain_mat(0,1);
    strain_vec[4] = 2*strain_mat(1,0);
    strain_vec[5] = 2*strain_mat(0,2);
...
Eigen::Matrix3d stressVoigt2Mat(const Vector6d& stress_vec)
{
    ...
    stress_mat <<       stress_vec[0], stress_vec[3], stress_vec[4],
                        stress_vec[3], stress_vec[1], stress_vec[5],
                        stress_vec[4], stress_vec[5], stress_vec[2];
    return stress_mat;
}
...
Vector6d stressMat2Voigt(const Eigen::Matrix3d& stress_mat)
{
    ...
    stress_vec[3] = stress_mat(0,1);
    stress_vec[4] = stress_mat(1,0);
    stress_vec[5] = stress_mat(0,2);
...
std::tuple<Vector6d, Matrix6d, PlasticState> radialReturn(const Vector6d& new_strain, const PlasticState& last_state)
{
    ...
    if (f_new_trial <= 0)
    {
        ...
        Matrix6d C_ep = Matrix6d::Zero();
        return std::make_tuple(stress, C_ep, new_state);
    }
...
{   
    ...
    for (double t = 0.0; t <= t_end; t+=dt)
    {
        ...
        const auto [stress, C_ep, new_plastic_state] = radialReturn(cur_strain, plastic_state);
        stresses.push_back(stress);
        plastic_states.push_back(new_plastic_state);
        C_eps.push_back(C_ep); 
        times.push_back(t);
    }
    ...
    plt.multiplot(1, 3, "HW6 (a)");
    // plot 1 - sigma vs time
    plt.set_title("sigma vs. time");
    plt.set_xlabel("Time");
    plt.set_ylabel("sigma");
    std::vector<double> sigma_11, sigma_22, sigma_12, sigma_33;
    for (const auto& stress : stresses) { sigma_11.push_back(stress[0]); sigma_22.push_back(stress[1]); sigma_12.push_back(stress[3]); sigma_33.push_back(stress[2]); }
    ...
    for (const auto& state : plastic_states) { beta_11.push_back(state.beta[0]); beta_22.push_back(state.beta[1]); beta_12.push_back(state.beta[3]); beta_33.push_back(state.beta[2]); }
